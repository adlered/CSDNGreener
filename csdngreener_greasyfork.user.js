...
class Progress {
  ...
  incProgress(p) {
    ...
    progress = 100;
    NProgress.done();
    $("#greenerProgress").html(
      protect_svg + " CSDNGreener 正在守护您的浏览体验"
    );
    setTimeout(function () {
...
if (isFirefox()) {
  ...
  $("head").append(
    "<style>.configContainer label{font-size:12px}.configContainer p{font-size:12px}.giveMeOneStar{font-size:15px}.configContainer .title{font-size:20px}.configContainer .bold{font-weight:bold;margin-bottom:5px}</style>"
  );
}
// SVG
...
(function (jQuery) {
  jQuery.fn.showTips = function (options, elem) {
    ...
    return this.each(function () {
      ...
      var pmr =
        opts.customid == "" ? Mathrandom : opts.customid.replace(/[#.]/, "");
      var pointer =
        opts.alignTo.length === 1
          ? "" + opts.alignTo[0] + ""
          : "" + opts.alignTo[0] + "-" + opts.alignTo[1] + "";
      ...
      if (pointer == "follow") {
        tfi.css({
          "border-bottom-color": opts.color[0],
          left: "" + offpos + percen + "",
          bottom: "100%",
        });
        tfem.css({
          ...
          left:
            "" + (offpos + (opts.alignTo[1] == "center" ? 0 : 1)) + percen + "",
          bottom: "100%",
        });
      } else {
        tfi.css(edgecolor, opts.color[0]).css(poi, "" + offpos + percen + "");
        tfem
          .css(edgecolor, opts.color[1])
          .css(
            poi,
            "" + (offpos + (opts.alignTo[1] == "center" ? 0 : 1)) + percen + ""
          );
      ...
      if (opts.isclose) {
        jQuery(
          '<span class="' + skin + 'close" id="close' + pmr + '">&times;</span>'
        ).appendTo(tipId);
      ...
      if (typeof opts.width === "string") {
        docW = parseInt(
          document.body.clientWidth * (opts.width.replace("%", "") / 100)
        );
      ...
      function showPosition(pointer, cell) {
        ...
        switch (pointer) {
          ...
          case "top-center":
            tipCell.css({
              top: post - tipH - spa,
              left: posl - tipW / 2 + selfW / 2,
            });
            break;
          case "top-right":
            tipCell.css({
              top: post - tipH - spa,
              left: posl - (tipW - selfW),
            });
            break;
          case "bottom-left":
...
(function () {
  ...
  // 点击遮罩关闭配置窗口点击事件
  let closeJss = "";
  closeJss += "<script>function closeConfig() {";
  closeJss += "$('.white_content').fadeOut(300);";
  closeJss += "$('#fade').fadeOut(300);";
  closeJss += "$('body').css('overflow', 'auto');";
  closeJss += "$('body').css('filter','none');";
  closeJss += "$('body').css('pointer-events','auto')";
  closeJss += "}</script>";
  $("body").append(closeJss);

  // 保存按钮点击事件
  let saveJss = "";
  saveJss += "<script>function saveAndReload() {";
  saveJss += "$('#configContent').fadeOut(200);";
  saveJss += "$('#fade').fadeOut(200);"; // 同时隐藏黑色遮罩层
  saveJss += "$('body').css('filter','none');"; // 移除模糊效果
  saveJss += "$('body').css('pointer-events','auto');"; // 恢复指针事件
  ...
  setTimeout(function () {
    ...
    if (main.test(currentURL) || mainNav.test(currentURL)) {
      ...
      // 顶栏VIP选项
      $(".toolbar-subMenu-box")
        .find("[href='https://mall.csdn.net/vip']")
        .parent()
        .remove();
      // CSDN工具广告
      put("#floor-ad_64");
      ...
      (blog.test(currentURL) && blockURL === 4) ||
      blog2.test(currentURL)
    ) {
      l("正在优化个人博客主页体验...");
      // 常规
      // 头部广告
      ...
      article.test(currentURL) &&
      !mp.test(currentURL) &&
      !article_month.test(currentURL)
    ) {
      l("正在优化阅读体验...");
      // 绿化设定
      if (isFirefox()) {
        setTimeout(function () {
          $(".toolbar-container-middle").prepend(
            "<div id='greenerProgress' style='text-align:right'></div>"
          );
          let htmlOf0 =
      ...
      // 右侧悬浮栏除置顶以外的按钮
      put(
        ".option-box[data-type='guide'],.option-box[data-type='cs'],.csdn-common-logo-advert"
      );
      // 登录后您可以享受以下权益
...
function loop(num) {
  setInterval(function () {
    if (num === 1) {
      ...
      // 主页广告
      $("li > div > div > h2 > a[href*='https://edu.csdn.net']")
        .parent()
        .parent()
        .parent()
        .parent()
        .remove();
      $("li > div > div > h2 > a[href*='https://marketing.csdn.net']")
        .parent()
        .parent()
        .parent()
        .parent()
      ...
      // 循环删除登录提示框
      if (
        $($(".passport-login-container")[0]).length == 1 &&
        deletedLogin == false
      ) {
...
function common(num, times) {
  var loop = setInterval(function () {
    ...
    if (num === 1) {
      ...
      // 页头广告
      try {
        document.getElementsByClassName("column-advert-box")[0].style.display =
          "none";
      } catch (e) {}
      ...
      $("a").each(function () {
        if (
          $(this).attr("href") === "https://passport.csdn.net/account/login"
        ) {
          $(this).html("登入");
        }
      });
      ...
      configHTML += "<br>";
      configHTML +=
        '<input type="checkbox" id="toggle-writeblog-button"> <label for="toggle-writeblog-button" class="modeLabel">显示发布按钮</label>';
      configHTML += "<br>";
      configHTML +=
        '<hr style="height:1px;border:none;border-top:1px solid #cccccc;margin: 5px 0px 5px 0px;" />';
      configHTML += '<p class="bold"><b>右侧栏定制</b></p>';
      configHTML +=
        '<input type="checkbox" id="toggle-ad-button"> <label for="toggle-ad-button" class="modeLabel">显示来自脚本的小广告（暂无）</label>';
      configHTML += "<br>";
      configHTML +=
      ...
      configHTML += "<br><br>";
      configHTML +=
        '<div><b>要不要来看看 :)</b><p>（作者本人建设的社区～</p><p>社区中聚集了同行业的大佬小白，欢迎小伙伴们一起摸鱼！</p><a href="https://fishpi.cn" target="_blank"><img src="https://s2.loli.net/2022/01/05/1HpBZUraMcR8ist.png" style="width:100%;height:100%;"/></a></div>';
      configHTML +=
        '<br><a href="https://doc.stackoverflow.wiki/web/#/21?page_id=138" target="_blank" class="giveMeOneStar">☕ 如果您觉得脚本好用，欢迎请我喝杯咖啡，我将努力更新脚本功能 ❤️</a><br>';
      configHTML +=
      ...
      // 绿化器设定
      $("body").after(
        '<div id="configContent" class="white_content">' +
          saveButton +
          configHTML
      );
      ...
      setTimeout(function () {
        ...
        // 阻止白色内容区域的点击事件冒泡到遮罩层
        $(".white_content").on("click", function (e) {
          e.stopPropagation();
        });
      }, 1000);
      ...
      if (whiteThemeCookie) {
        ...
        $(
          '[href^="https://csdnimg.cn/release/phoenix/template/themes_skin/"]'
        ).attr(
          "href",
        ...
        $(".csdn-logo").attr(
          "src",
          "//csdnimg.cn/cdn/content-toolbar/csdn-logo.png?v=20200416.1"
        );
      ...
      if (searchBlogCookie) {
        $("#recommend-right").append(
          $("#asideSearchArticle").prop("outerHTML")
        );
        setTimeout(function () {
          $("#asideSearchArticle").attr(
            "style",
          ...
          n.on("click", function (t) {
            ...
            if (n) {
              var e =
                "//so.csdn.net/so/search/s.do?q=" + n + "&t=blog&u=" + username;
              window.open(e);
            }
            t.preventDefault();
          });
      ...
      if (!kindPersonCookie) {
        ...
        if ($("#asideCategory").length > 0) {
          ...
          $(".kind_person").attr(
            "style",
            "margin-top: 8px; width: 300px; height:255px;"
          );
        }
        setTimeout(function () {
          $("#asideCategory").attr(
            "style",
            "margin-top: 8px; width: 300px; display:block !important;"
          );
      ...
      // 评论复制按钮
      $(".comment-box").prepend(
        '<button class="comment-hidden-text" style="display:none">COPY BUTTON</button>'
      );
